...
icon-rounded.svg        — Mesma arte com CANTOS ARREDONDADOS (clipPath rx=184 ≈ 18%; cantos transparentes fora do round-rect). Base dos PNGs `purpose:"any"`. Gerada a partir de icon.svg (wrap do <g> num clip-path)
...
**Service Worker:** incrementar `CACHE_NAME` em `service-worker.js` a cada deploy com mudanças de cache. Versão atual: `gentehonesta-v168`. Os arquivos CSS e JS são atualizados automaticamente pelo Network-First; o incremento serve para forçar limpeza de caches antigos.
