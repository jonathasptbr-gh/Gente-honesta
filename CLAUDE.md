...
icon-192.png            — Ícone PWA 192px (operário: capacete + óculos + rosto + check, cores do tema); referenciado no manifest.json
...
**Service Worker:** incrementar `CACHE_NAME` em `service-worker.js` a cada deploy com mudanças de cache. Versão atual: `gentehonesta-v151`. Os arquivos CSS e JS são atualizados automaticamente pelo Network-First; o incremento serve para forçar limpeza de caches antigos.
