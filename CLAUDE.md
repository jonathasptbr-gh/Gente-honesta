...
icon.svg                — Fonte do ícone do app (capacete de obras + check, cores do tema); renderizado para os PNGs
icon-192.png            — Ícone PWA 192px (referenciado no manifest.json)
icon-512.png            — Ícone PWA 512px (referenciado no manifest.json)
...
**Service Worker:** incrementar `CACHE_NAME` em `service-worker.js` a cada deploy com mudanças de cache. Versão atual: `gentehonesta-v149`. Os arquivos CSS e JS são atualizados automaticamente pelo Network-First; o incremento serve para forçar limpeza de caches antigos.
