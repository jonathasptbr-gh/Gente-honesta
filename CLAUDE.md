...
para distribuir a altura extra): cabeçalho (eyebrow dourado "SUA REPUTAÇÃO NA PLATAFORMA" + título branco);
`.ic-hero` (70% GIGANTE em degradê dourado via `background-clip: text` + status "Em formação" + frase de
responsabilidade — o índice começa em 70% e mantê-lo depende do usuário); `.ic-meter` (barra segmentada nas
4 faixas com pino "VOCÊ" pulsante — `icPinPulse` — posicionado em `left: 70%`, + `.ic-meter__zones` com os
MESMOS escudos do resto do app `gpp_bad`/`gpp_maybe`/`shield_question`/`gpp_good` em versões de cor legíveis
no fundo escuro; a zona atual `.ic-zone--current` acende, as demais ficam com `opacity: 0.78`); `.ic-factors`
(duas colunas "Faz subir"/"Faz cair", resumo do diálogo de ajuda `btn-ic-info`); e `.ic-card__footer`
(lema "confiança se constrói devagar — e se perde rápido"). `.form-group--ic-fill` = `flex: 1 0 auto`
(preenche o vão até o botão de concluir). **Adaptativo por altura** (`@media (max-height)` em
`#view-onboarding`, limiares MEDIDOS por estado: completo ~889px, sem rodapé ~823px, sem colunas ~744px,
compacto ~693px): ≤896px esconde o rodapé; ≤830px esconde as colunas e aperta o padding; ≤751px compacta
(some a frase do hero e os nomes das zonas, encolhem 70% e escudos) — cabe sem rolagem de ~693px pra cima.
...
**Service Worker:** incrementar `CACHE_NAME` em `service-worker.js` a cada deploy com mudanças de cache. Versão atual: `gentehonesta-v189`. Os arquivos CSS e JS são atualizados automaticamente pelo Network-First; o incremento serve para forçar limpeza de caches antigos.
